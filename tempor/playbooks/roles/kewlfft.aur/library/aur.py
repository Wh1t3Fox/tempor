...
def install_with_makepkg(
    module, package, extra_args, skip_pgp_check, ignore_arch, local_pkgbuild=None
):
    """
...
def install_local_package(module, package, use, extra_args, local_pkgbuild):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        shutil.copytree(local_pkgbuild, tmpdir, dirs_exist_ok=True)
        command = build_command_prefix(
            use, extra_args, local_pkgbuild=tmpdir + "/PKGBUILD"
        )
...
def upgrade(module, use, extra_args, aur_only):
    ...
    module.exit_json(
        changed=not (
            out == "" or "nothing to do" in out or "No AUR updates found" in out
        ),
        msg="upgraded system",
        helper=use,
    )
...
def install_packages(
    ...
    for package in packages:
        ...
        changed_iter = changed_iter or not (
            out == "" or "-- skipping" in out or "nothing to do" in out
        )
...
def make_module():
    ...
    if use == "auto":
        if params["extra_args"] is not None:
            module.fail_json(
                msg="'extra_args' cannot be used with 'auto', a tool must be specified."
            )
        use = "makepkg"
    ...
    if params["local_pkgbuild"] and not os.access(
        params["local_pkgbuild"] + "/PKGBUILD", os.R_OK
    ):
        module.fail_json(
            msg="PKGBUILD inside %s not readable" % (params["local_pkgbuild"])
        )
